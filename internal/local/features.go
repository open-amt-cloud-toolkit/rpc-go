...
func (service *ProvisioningService) SetAMTFeatures() error {
    ...
    amtFeatures := service.config.Configure.AMTFeatures
	// Determine the redirection state
	isRedirectionChanged := amtFeatures.KVM || amtFeatures.SOL || amtFeatures.IDER
	...
	isISMSystem := false //ToDO: Work on it 
	if err != nil {
	...
	if isISMSystem && amtFeatures.KVM {
	...
	switch amtFeatures.UserConsent {
	case "none":
	...
	if !isISMSystem {
		println("KVM Enabled		:", amtFeatures.KVM)
	}
	println("SOL Enabled		:", amtFeatures.SOL)
	println("IDER Enabled		:", amtFeatures.IDER)
	println("User Consent		:", amtFeatures.UserConsent)
...
func (service *ProvisioningService) setAMTRedirectionService() error {
	...
	if service.config.Configure.AMTFeatures.IDER {
		requestedState += 1
	}
	if service.config.Configure.AMTFeatures.SOL {
		requestedState += 2
	}
