...
func (service *ProvisioningService) Configure() (err error) {
	...
	service.interfacedWsmanMessage.SetupWsmanClient("admin", service.config.Configure.AMTPassword, logrus.GetLevel() == logrus.TraceLevel)

	switch service.config.Configure.Subcommand {
