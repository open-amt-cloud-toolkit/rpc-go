...
func createConfigureCommand() *cobra.Command {
	...
	wifiCfg := config.WifiProfile{}
	ieee8021xCfg := config.Ieee8021xProfile{}
...
func runAddWifiSettings(cmd *cobra.Command, args []string) error {
	...
	if configPathOrString != "" {
		...
		config.WifiProfiles = append(config.WifiProfiles, wifiCfg)
		...
		config.Ieee8021xProfiles = append(config.Ieee8021xProfiles, ieee8021xCfg)
	}
...
func readWiFiSettings(configString string, config *config.Config) error {
...
func readWiFiConfigFile(configPathOrString string, config *config.Config) error {
